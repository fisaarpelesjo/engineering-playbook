...
| 6 | Vinculo sub-issue para issue mae e issue mae para especificacao | CI | coberta, T212, `src/engineering_playbook/delivery.py` (`traceability_refusal`), `tests/unit/test_the_chain_is_verified.py`: a issue que a pull request fecha resolve por `parent_issue_url` ate uma issue mae que nomeia `specs/NNN/` existente, ou nomeia-a ela propria, ou declara `no_spec_reason`. Elo em falta e nomeado. Alargado em #65: o corpo da issue nao pode afirmar `Sub-issue of #N` sem que a API reporte esse pai. O `traceability_refusal` pergunta se existe pai; esta verificacao pergunta se o pai que o texto anuncia e o que a estrutura tem -- medido em 2026-09-22, duas issues abertas pelo agente deste repositorio afirmavam-no sem qualquer relacao, e foram ligadas a mao. Refs: FR-015, AC-009 |
...
| 9 | Ordem `start`, `prepare`, `commit`, `publish`, `merge` | parcial, `publish` e `merge` nao verificam recibo do antecessor | FR-005, AC-003. T218 acrescentou a precondicao de `start`: recusa partir de HEAD que a base nao contem, com `--from-base` como saida. T219 fechou o segundo referente: `resolve_base` devolve `refs/remotes/<remote>/<base>`, ou `refs/heads/<base>` declarado quando nao existe ref remoto, e `publish` e `status` consomem-no, com teste ponta a ponta por comando e duas entradas de mutacao sobre os sitios de chamada (#61). #64 fechou um defeito diferente da mesma etapa: `publish` escrevia `.project/state.yml` e o checkpoint DEPOIS do push e deixava-os por commitar, e como `start` recusa arvore suja o pipeline bloqueava a propria fatia seguinte. `commit_bookkeeping` dobra essas escritas num commit proprio, simetrico com o que o `commit` ja fazia pela FR-001, com quatro entradas de mutacao (T227). O recibo do CI local saiu dos registos e passou a ignorado: nomeia a cabeca que verificou, portanto o segundo push regenera-o contra o commit que transporta. Permanece parcial por duas razoes, ambas declaradas: `publish` e `merge` ainda nao verificam recibo do antecessor; e o `merge` continua a deixar arvore suja. A razao desta segunda foi corrigida por revisao, que mostrou que a primeira versao dela nao decorria da evidencia citada. O que prende e: o registo pos-merge (SHA do squash e arvore) nao e conhecivel antes do merge; depois do merge nao alcanca main, porque o ruleset `protect-main` exige pull request com `bypass_actors` vazio; dobra-lo localmente sem push e possivel, e foi a rota que eu nao tinha visto, mas deixaria o registo orfao numa branch cujo remoto o proprio `merge` apaga; e antecipar a escrita para antes do `gh pr merge` moveria a cabeca depois de `signed_verdict_refusal` ter medido `headRefOid`, trocando arvore suja por conteudo que o portao de merge nao mediu -- que e a razao mais forte das quatro e nao estava escrita. T228 (#73) closed the stage's first link: `start` records `issue` and the branch in the state, and `prepare` refuses a state whose `issue` is not the number the branch was started for, before the gates; three mutation entries. Declared cost: `start` leaves `.project/state.yml` modified, so a second `start` needs `--allow-dirty` |
| 10 | Formatacao, analise estatica, tipos e testes | servidor | coberta, `.github/workflows/quality.yml`, `.github/rulesets/main.yml`, `tests/unit/test_local_ci.py`, `tests/unit/test_the_gate_tools_match_the_lock.py`. #76: the ruff and pyright that ran are the versions `uv.lock` names, measured inside `pytest` so the pre-push hook and CI each refuse on their own, and `pyproject.toml` pins them with `==` so the lock cannot move without a reviewed edit; two mutation entries |
| 11 | Schemas, distribuicao e fixacao de Actions por SHA | CI | coberta, `core.verify_root` (GitHub Actions must be pinned by full SHA), `tests/unit/test_schema_validation.py`, `tests/unit/test_resource_mirror_parity.py`. A evidencia estava sobredeclarada ate #49: a paridade do espelho citada aqui excluia `.project/schemas/*` e `.github/workflows/quality.yml`, isto e, as duas familias com maior probabilidade de derivar, e o workflow foi editado a mao varias vezes numa sessao enquanto a exclusao vigorava -- sem numero exacto, porque o `git log` ve commits e nao sessoes e nenhum instrumento aqui o reproduz. Medido em 2026-09-22: os sete schemas ja eram identicos, portanto a exclusao nao protegia nada e apenas escondia a proxima edicao; o workflow difere em dois pontos legitimos, o job `adversarial` que um projecto derivado nao consegue correr e o `needs:` do `attest`, e passou a ser comparado de duas maneiras redundantes: estruturalmente, com o YAML das duas copias comparado depois de remover esse job, o que e imune a formatacao; e textualmente, entre ancoras literais asseridas como unicas, o que apanha comentarios e espacos em branco que o YAML nao ve. A primeira versao desta comparacao removia o bloco por varrimento de indentacao e a revisao mediu-a a falhar aberta nos dois sentidos com uma unica edicao. ALCANCE DECLARADO, NFR-005: nao sao comparadas as linhas em branco da costura -- no ficheiro da raiz sao as que precedem `  adversarial:`, isto e, o fim do job anterior, e na copia distribuida as que precedem `  attest:`. Pertencem ao bloco removido e no espelho nao tem onde viver; sem esta cegueira, espelhar correctamente essa linha dava vermelho. Uma linha com espacos nao conta como em branco, portanto continua a ser comparada; e a comparacao e sobre este par de ficheiros, nao sobre a adequacao do workflow. Inclui tambem, desde #65, o guarda de idioma: `tests/unit/test_the_repository_speaks_one_language.py` le os ficheiros rastreados pela deteccao partilhada em `src/engineering_playbook/language.py` e fixa nos dois sentidos o inventario dos que ainda falam portugues -- esta linha nomeia-o porque a entrada de mutacao correspondente esta arquivada nesta etapa e um leitor que audite a FR-016 pela matriz nao o encontrava em lado nenhum. 85 pares espelhados, e quatro entradas no inventario de mutacao -- uma por cada metade da redundancia mais a paridade dos schemas, porque a revisao mediu que a comparacao textual, citada aqui como metade do mecanismo, nao tinha entrada nenhuma a prova-la |
...
| Mecanismo removido sem que nenhum teste o note | parcial, alcance declarado | T215/T216: `src/engineering_playbook/mutation.py` remove cada mecanismo do inventario numa copia, exige que os alvos estivessem verdes antes, e so conta como apanhado um node id que passou a falhar. Medido em 2026-09-22, apos #64: 39 de 39 apanhados, e o inventario alcanca 10 das 20 etapas desta matriz -- 4, 6, 9, 10, 11, 15, 16, 17, 18, 20. As restantes nao tem entrada, portanto o vector permanece aberto para elas. Corre como job proprio em integracao continua (AC-013), e o job de assinatura depende dele |
