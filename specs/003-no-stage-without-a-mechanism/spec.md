...
| Criacao de branch fora da esteira (`git switch -c`, `git checkout -b`, `git branch`) | fechado | Medido contra `.claude/hooks/enforce_delivery_pipeline.py` em 2026-09-21: `WRITE_VERB` cobre `commit|push|merge|rebase|reset` e `branch` esta na lista de leitura. Fecha com T221, que depende de T218 ter dado ao operador `--from-base` |
